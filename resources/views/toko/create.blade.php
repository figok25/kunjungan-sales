    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-lg p-6">
                <form method="POST" action="{{ route('toko.store') }}" class="space-y-4">
                    @csrf

                    <div>
                        <label class="block text-sm text-gray-600 mb-1">Nama Toko</label>
                        <input type="text" name="nama_toko" value="{{ old('nama_toko') }}" class="w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <div>
                        ...
                        <textarea name="alamat" rows="3" class="w-full border-gray-300 rounded-md shadow-sm">{{ old('alamat') }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="text" name="no_telp" value="{{ old('no_telp') }}" class="w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <div>
                        ...
                        <select name="sales_id" class="w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($salesList as $sales)
                                <option value="{{ $sales->id }}" @selected(old('sales_id') == $sales->id)>
                                    {{ $sales->name }}
                                </option>
                            @endforeach
                        </select>
